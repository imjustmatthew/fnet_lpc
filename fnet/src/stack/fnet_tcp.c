...
static int fnet_tcp_sendseg(struct fnet_tcp_segment *segment)
{
    ...
    /* Create the header.*/
    segment_buf = fnet_netbuf_new(FNET_TCP_SIZE_HEADER, FNET_FALSE);
    ...
    fnet_memset_zero(segment_buf->data_ptr, FNET_TCP_SIZE_HEADER);
    ...
    FNET_TCP_SPORT(segment_buf) = segment->src_addr.sa_port;
    FNET_TCP_DPORT(segment_buf) = segment->dest_addr.sa_port;
    ...
    {
        FNET_TCP_CHECKSUM(segment_buf) = 0; 
        FNET_TCP_CHECKSUM(segment_buf) = fnet_checksum_pseudo_start(segment_buf, FNET_HTONS((unsigned short)FNET_IP_PROTOCOL_TCP), (unsigned short)segment_buf->total_length);
    }
    ...
    {
        error = fnet_ip_output(0, ((struct sockaddr_in *)(&segment->src_addr))->sin_addr.s_addr, 
    ...
    {
        /* Check Scope ID.*/
        fnet_netif_t *netif = fnet_netif_get_by_scope_id( ((struct sockaddr_in6 *)(&segment->dest_addr))->sin6_scope_id );
