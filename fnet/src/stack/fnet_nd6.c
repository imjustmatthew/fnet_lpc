...
void fnet_nd6_neighbor_solicitation_send(fnet_netif_t *netif /*MUST*/, fnet_ip6_addr_t *ipsrc /* NULL for, DAD */, fnet_ip6_addr_t *ipdest /*set for NUD,  NULL for DAD & AR */, fnet_ip6_addr_t *target_addr)
{
    ...
    if((ns_nb = fnet_netbuf_new((int)ns_packet_size, FNET_TRUE)) != 0)
    {
        ...
        ns_packet->icmp6_header.checksum = 0;
        ns_packet->icmp6_header.checksum = fnet_checksum_pseudo_start( ns_nb, FNET_HTONS((unsigned short)FNET_IP_PROTOCOL_ICMP6), (unsigned short)ns_nb->total_length );
        
        fnet_ip6_output( netif, ipsrc, ipdest, FNET_IP_PROTOCOL_ICMP6, FNET_ND6_HOP_LIMIT, ns_nb, &ns_packet->icmp6_header.checksum);
    }
...
void fnet_nd6_neighbor_advertisement_send(fnet_netif_t *netif, fnet_ip6_addr_t *ipsrc, fnet_ip6_addr_t *ipdest, unsigned char na_flags)
{
    ...
    if((na_nb = fnet_netbuf_new((int)na_packet_size, FNET_TRUE)) != 0)
    {
        ...
        /* Checksum calculation.*/
        na_packet->icmp6_header.checksum = 0;
        na_packet->icmp6_header.checksum = fnet_checksum_pseudo_start( na_nb, FNET_HTONS((unsigned short)FNET_IP_PROTOCOL_ICMP6), (unsigned short)na_nb->total_length );
        
        fnet_ip6_output( netif, ipsrc, ipdest, FNET_IP_PROTOCOL_ICMP6, FNET_ND6_HOP_LIMIT, na_nb, &na_packet->icmp6_header.checksum);
    }
...
void fnet_nd6_router_solicitation_send(fnet_netif_t *netif) 
{
    ...
    {   
        ...
        /* Checksum calculation.*/
        rs_packet->icmp6_header.checksum = 0;
        rs_packet->icmp6_header.checksum = fnet_checksum_pseudo_start( rs_nb, FNET_HTONS((unsigned short)FNET_IP_PROTOCOL_ICMP6), (unsigned short)rs_nb->total_length );
        
        fnet_ip6_output( netif, ip_src, ip_dest, FNET_IP_PROTOCOL_ICMP6, FNET_ND6_HOP_LIMIT, rs_nb, &rs_packet->icmp6_header.checksum);
    }
...
void fnet_nd6_router_advertisement_receive(fnet_netif_t *netif, fnet_ip6_addr_t *src_ip, fnet_ip6_addr_t *dest_ip, fnet_netbuf_t *nb, fnet_netbuf_t *ip6_nb)
{
    ...
    *************************************************************/	
    ...
    {   
        ...
        if(nd_option_mtu) 
        {
            netif->nd6_if_ptr->mtu = fnet_ntohl(nd_option_mtu->mtu);
            if(netif->nd6_if_ptr->mtu < FNET_IP6_DEFAULT_MTU)
